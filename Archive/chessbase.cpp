...
namespace CHESSBASE
{
    ...
    int Try_Move(const animal_type & chesscode, const step_type & direction, const MAP (*map)[7], const MY_ANIMAL cc[], const ENEMY dd[], bool & start_chain, const int & repeat_ch, bool & can_change)
    {
        ...
        else
        {
            ...
            if (half == 3 && chesscode != mouse)
                disv = Vdis_New(to, chesscode, map, cc, dd);
            
            if (half == 1 && chesscode == lion)
                big += 10;
            
            if (half == 2 && chesscode == elephant && cc[elephant - 1].x < 3){ big = 10; disv = Vdis_New(to, elephant, map, cc, dd);}
            
            if (half == 4 && In_Danger_Simple(animal_type(repeat_ch + 1), map, cc, dd) == 0 && chesscode != animal_type(repeat_ch + 1) && (chesscode == lion || chesscode == tiger))
                disv += 15;
            
            
            if (chesscode == wolf && dd[mouse - 1].isAlive) disv = 2;
            ...
            if (To_Danger(to, chesscode, map, cc, dd) && Vdis(to) < 900 && !(chesscode == mouse && Yammy(to, chesscode, map, cc, dd) > 7) /*&& Make_Chance(to, chesscode, map, cc, dd) == 0*/) return 1;
            ...
            if (Stop_For_Defense(chesscode, map, cc, dd) && Yammy(to, chesscode, map, cc, dd) < 100) {start_chain = false; if (int(chesscode) - 1 == repeat_ch) can_change = false; return 1;}
            ...
            return std::max(disv, Vdis_Special(to, chesscode, map, cc, dd)) - In_Danger_Simple(chesscode, map, cc, dd) - In_Danger_Special(chesscode, map, cc, dd) + Yammy(to, chesscode, map, cc, dd) + big + To_Yammy(to, chesscode, map, cc, dd) + To_Help_Other_Specail(to, chesscode, map, cc, dd) + jumpbonus + To_Help(to, chesscode, map, cc, dd) + rush_bonus /*+ Make_Chance(to, chesscode, map, cc, dd)*/ + adj;
        }
    }
            ...
            if (to.x - 1 >= 0)
            {
                ...
                if (tea != nanan)
                {
                    if (cc[tea - 1].x == to.x - 1 && cc[tea - 1].y == to.y && cc[tea - 1].isAlive)
                        if (Can_Eat(tottea, tea, map, cc, to.x, to.y))
                            return 7 - (int(tea) - int(tottea));
                }
            }
            if (to.x + 1 <= 8)
            {
